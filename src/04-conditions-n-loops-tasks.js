...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (right1 < rect2.left || rect1.left > right2 || bottom1 < rect2.top || rect1.top > bottom2) {
    return false;
  }
...
function findFirstSingleChar(str) {
  ...
  const uniqueChar = chars.find((char) => count[char] === 1);
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(String(num).split('').map(Number).reduce((acc, digit) => acc + digit, 0));
}
...
function toNaryString(num, n) {
  if (num === 0) return '0';
  if (n < 2 || n > 10) throw new Error('Base must be between 2 and 10');
  let result = '';
  let current = num;
  while (current > 0) {
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    ...
    for (let j = 1; j < pathes.length; j += 1) {
      const pathParts = pathes[j].split('/');
      if (pathParts[i] !== currentPart) {
