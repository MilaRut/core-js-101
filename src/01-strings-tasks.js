...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (punctuationRegex.test(char) || char === ' ') {
      ...
      const indexOfChar = alphabet.indexOf(lowerChar);
      const newCharIndex = (indexOfChar + 13) % 26;

      if (isUpperCase) {
        res.push(alphabet[newCharIndex].toUpperCase());
      } else {
        res.push(alphabet[newCharIndex]);
      }
    }
  }
